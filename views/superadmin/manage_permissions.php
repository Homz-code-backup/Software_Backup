                                                            ...
                                                            <div class="sub-card d-flex align-items-center justify-content-between mb-2">
                                                                <div class="flex-grow-1">
                                                                    <div class="fw-semibold small d-flex align-items-center gap-2">
                                                                        <?= $sp['permission_name'] ?>
                                                                        <?php
                                                                        // Check if this sub-permission has city constraint
                                                                        $hasCity = !empty($sp['has_city_constraint']);
                                                                        // Check if this sub-permission has branch constraint
                                                                        $hasBranch = !empty($sp['has_branch_constraint']);

                                                                        if ($hasCity): ?>
                                                                            <span class="badge bg-info text-white" style="font-size:9px" title="Requires City Selection">
                                                                                <i class="bi bi-geo-alt-fill"></i> CITY
                                                                            </span>
                                                                        <?php endif; ?>

                                                                        <?php if ($hasBranch): ?>
                                                                            <span class="badge bg-warning text-dark" style="font-size:9px" title="Requires Branch Selection">
                                                                ...
                                                                <div class="btn-group gap-1">
                                                                    <button class="btn btn-sm btn-outline-warning edit-sub"
                                                                        ...
                                                                        data-slug="<?= $sp['slug'] ?>"
                                                                        data-city="<?= $sp['requires_city_scope'] ?? 0 ?>"
                                                                        data-branch="<?= $sp['requires_branch_scope'] ?? 0 ?>">
                                                                        <i class="bi bi-pencil"></i>
                                                                    </button>
    ...
    <div class="modal-dialog">
        <form method="POST" action="../superadmin/manage-permissions/page" class="modal-content">
            <div class="modal-header">
                <h5>Add Page</h5><button class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="editPageModal">
    <div class="modal-dialog">
        <form method="POST" action="../superadmin/manage-permissions/page" class="modal-content">
            <div class="modal-header">
                <h5>Edit Page</h5><button class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
...
<div class="modal fade" id="addSubModal">
    <div class="modal-dialog">
        <form method="POST" action="../superadmin/manage-permissions/sub" class="modal-content">
            <div class="modal-header">
                <h5>Add Sub Permission</h5><button class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <input type="hidden" name="page_id" id="as_page">
                <input class="form-control mb-2" name="permission_name" placeholder="Name" required>
                <input class="form-control mb-3" name="slug" placeholder="view | edit" required>

                <div class="border-top pt-3 mt-2">
                    <label class="form-label fw-semibold small">Scope Requirements</label>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="requires_city_scope" id="add_city_scope">
                        <label class="form-check-label small" for="add_city_scope">
                            <i class="bi bi-geo-alt-fill text-info"></i> Requires City Selection
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="requires_branch_scope" id="add_branch_scope">
                        <label class="form-check-label small" for="add_branch_scope">
                            <i class="bi bi-building text-warning"></i> Requires Branch Selection
                        </label>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="editSubModal">
    <div class="modal-dialog">
        <form method="POST" action="../superadmin/manage-permissions/sub" class="modal-content">
            <div class="modal-header">
                <h5>Edit Sub Permission</h5><button class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <input type="hidden" name="id" id="es_id">
                <input class="form-control mb-2" name="permission_name" id="es_name" required>
                <input class="form-control mb-3" name="slug" id="es_slug" required>

                <div class="border-top pt-3 mt-2">
                    <label class="form-label fw-semibold small">Scope Requirements</label>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="requires_city_scope" id="es_city_scope">
                        <label class="form-check-label small" for="es_city_scope">
                            <i class="bi bi-geo-alt-fill text-info"></i> Requires City Selection
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="requires_branch_scope" id="es_branch_scope">
                        <label class="form-check-label small" for="es_branch_scope">
                            <i class="bi bi-building text-warning"></i> Requires Branch Selection
                        </label>
                    </div>
                </div>
...
<script>
    ...
    document.querySelectorAll('.edit-sub').forEach(b => b.onclick = () => {
        ...
        es_city_scope.checked = b.dataset.city == '1';
        es_branch_scope.checked = b.dataset.branch == '1';
    });
